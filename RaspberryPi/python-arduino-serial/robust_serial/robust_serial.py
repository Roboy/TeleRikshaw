...
def decode_order(f, byte, debug=False):
    ...
    try:
        ...
        if order == Order.HELLO:
            msg = "HELLO"
        elif order == Order.DIRECTION:
            angle = read_i16(f)
            # Bit representation
            # print('{0:016b}'.format(angle))
            msg = "direction {}".format(angle)
        elif order == Order.SPEED:
            speed = read_i8(f)
            msg = "motor {}".format(speed)
        elif order == Order.CAMERA:
            camera_angle = read_i16(f)
            msg = "camera direction {}".format(camera_angle)
        elif order == Order.BUZZER:
            frequency = read_i16(f)
            msg = "frequency {}".format(frequency)
        elif order == Order.JINGLEBELLS:
            num = read_i16(f)
            msg = "number of jinglebells {}".format(num)
        elif order == Order.LED:
            color = read_i16(f)
            msg = "color {}".format(color)
        else:
            msg = ""
